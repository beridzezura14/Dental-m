...
const Header = () => {
  ...
  return (
    <>
      <header 
        className={`fixed top-0 left-0 w-full z-[120] transition-all duration-500 px-6 lg:px-16 ${
          scrolled 
            ? "py-4 bg-slate-950/90 backdrop-blur-md border-b border-white/5 shadow-lg" 
            : "py-8 bg-transparent"
        }`}
      >
      ...
      {/* Mobile Sidebar Overlay - გამოვიტანე ჰედერ გარეთ */}
      <div className={`fixed inset-0 bg-slate-950 z-[115] lg:hidden flex flex-col items-center justify-center gap-8 transition-all duration-500 ease-in-out ${
        ...
        {navItems.map((item, i) => (
          <a 
            ...
            className="text-white text-4xl font-black uppercase tracking-tighter hover:text-emerald-500 transition-all transform active:scale-90"
            style={{ 
        ...
        <a 
          ...
          style={{ opacity: isOpen ? 1 : 0, transitionDelay: '250ms' }}
        >
          ვიზიტის დაჯავშნა
        </a>
      </div>
